...
from tools.research_tools import arxiv_search_tool, wikipedia_search_tool, tavily_search_tool
import requests
...
# Arxiv tool
def tool_arxiv_search(args: Dict[str, Any]) -> Dict[str, Any]:
    if arxiv_search_tool is None:
        raise RuntimeError("tool_arxiv_search helper not available")
    query = args["query"]
    print(query)
...
# tavily search tool
def tool_tavily_search(args: Dict[str, Any]) -> Dict[str, Any]:
    if tavily_search_tool is None:
        raise RuntimeError("tavily_search_tool helper not available")
    query = args["query"]
    print(query)
...
# Wikipedia tool
def tool_wikipedia_search(args: Dict[str, Any]) -> Dict[str, Any]:
    if wikipedia_search_tool is None:
        raise RuntimeError("tool_wikipedia_search helper not available")
    query = args["query"]
    print(query)
    results_list = []
    results_list = wikipedia_search_tool(query)
...
def bootstrap_example(use_openai: bool = True) -> Agent:
    ...
    agent = Agent(
        ...
        tools=tools,
        memory=memory,
        config=AgentConfig(max_steps=5, reflection=False, debug=True),
    )
    return agent
